...
> **塞いだ（2026-08-29、決定1-14）。** 観測4本を読み直すと、**骨組み表示だけが3本**で
> 使われており（p 2 / h 40 / s 1）、`spin` や `accordion` は1本ずつしか無かった。
> `[data-sg-skeleton]` と `--animate-skeleton` を持ち、**周期はループスケールから引く。**
> `prefers-reduced-motion` は**トークン層が尊重する**——holosphere は `motion-reduce:` を
> 62 箇所書いていたが、**書かなくて済むほうがよい。**
>
> **イージングは値を持たない。4本のどこにも自作の `cubic-bezier` が無かった。**
> ただし `--*: initial` で `ease-in-out` だけが消えていた（`ease-linear` は静的
> ユーティリティなので生きている）ので、**語をそのまま戻した。**
...
> **宣言した（2026-08-28、決定5-5 改訂）。上限は 6。**
> 上限を決めるのは色相ではなく**段**だった。色相だけでは4色すら二色覚下で分けられず
> （5色の最良で 0.028、閾値 0.08）、決定5-8 の段の変化が識別を担っている。
> したがって**マークとして使える段の数**が上限になる——明色7段、暗色6段。
> **両モードで成立するのは 6。** 系列数 5 は変えていない。数の問題ではなかった。
> 上限を超える分類は `other` に畳むか、色以外のチャネルを併用する。
> **holosphere の10色目 `other` が中間色（slate）だったのは、まさにその形である。**
...
| G5 skeleton の出口が無い | **塞がった**（決定1-14、#47）。骨組み表示だけを持ち、動きを減らす設定もトークン層が尊重する |
| G6 識別色の上限 | **塞がった**（決定5-5 改訂、#48）。上限は 6 と宣言。系列数 5 は変えない |
