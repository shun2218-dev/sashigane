...
| 影（`shadow-lg` など） | 4 | 8 | 既知（当時 elevation 未実装、決定1-8）。**2026-08-29 に実装した**（Issue #81） |
| その他 | 1 | 1 | — |
...
| G7 JS から値を読めない | **塞がっている。** 利用側の適用が残っているだけ。**JS が要るのは色だけ**だと 2026-08-29 に確かめた（決定2-6、#71） |
...
## 追記（2026-08-29）— OG 画像がトークンから読むのは色だけだった

> [Issue #71](https://github.com/shun2218-dev/sashigane/issues/71) の判断。
> 決定2-6 は JS 経路の実需要として **OG 画像**を挙げていた。
> `tokens.js` は差し込み口の値を潰すので、**そこで書体と太さがずれるのではないか**と考えた。

**ずれない。** `src/app/api/og/song/route.tsx` と `src/og/fonts.ts` を読んだ。

| | 実装 | トークンから読むか |
|---|---|---|
| 書体 | `loadGoogleFont()` が Noto Sans JP を**サブセットしてバイナリで渡す** | **読まない。** satori は名前ではなくファイルを要求する |
| 太さ | ローダが `weight: 400 \| 700` の**2つだけ**取得する | **読まない。** 取得したファイルにある分しか出せない |
| 色 | `ACCENT = '#e879f9'` と直書き | **ここが需要** |

CJK を描くために**サブセットしたフォントファイルを渡す**構造なので、
書体名だけを渡しても意味が無い。太さも同じ制約を受ける。

**残る2本も読んだ。**

| | 実装 | 何を読むか |
|---|---|---|
| ichirizuka `IntensityStrip` | ブラウザで描くインライン SVG。`fontFamily="var(--mono)"` | **書体は CSS 変数から**。JS 側は色だけ |
| pylabo `CourseCard` | `style={{ '--accent': course.accent }}` | データの色を CSS 変数へ流し込む |

**3本とも需要は色である。** 色は差し込み口を持たないので、潰れて困る値がそもそも無かった。

**そして CSS が本当に届かないのは OG 画像だけだった。** 残る2本はブラウザで動いており、
書体は現に CSS 変数から読んでいる。**JS を使う理由は「値が実行時に決まる」ことであって、
CSS が届かないことではない。**

なお実装コメントは satori について「**flexbox と CSS の部分集合しか解さない**」と明言している。
色を16進で出し続ける理由が、互換性の推測ではなく**需要の側**から立った。
