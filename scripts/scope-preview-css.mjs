...
/** 1つのセレクタを限定する。**戻り値は複数になりうる**（`*` は目印自身も含める） */
const scopeOne = (sel) => {
  const s = sel.trim();
...
export const scopeCss = (css) => {
  ...
  return { css: root.toString(), scoped };
};
...
const { css, scoped } = scopeCss(before);
...
console.log(`✓ 対照 7 件が期待どおり`);
console.log(`✓ ${target} の ${scoped} 規則を ${SCOPE} の中に限定しました`);
