/**
 * 生成物が**コピー先で意味を成す**ことを検査する（原則6、Issue #34）。
 *
 * レジストリ方式では生成物が利用側リポジトリへコピーされる。そこには `docs/decisions.md` も
...
const EXCLUDED = [
  {
    ...
    why:
      ...
      'JSON にコメントは書けず、利用側リポジトリへもコピーされない',
  },
];
...
const REQUIRED = [
  ...
  {
    /**
     * コピー先で「どのスナップショットを持っているか」を知る唯一の手がかり（決定4-6）。
