 ...
 *
 * ## 過剰に検出するもの — **補間の中に入れ子の `}` があると読み切れない**
 *
 * `INTERPOLATION` は `\$\{[^}]*\}` なので、**最初の `}` で止まる。**
 *
 * class の属性値の中に、**入れ子の閉じ括弧を持つ補間**を書いた場合である
 * （cva の呼び出しをテンプレートリテラルの中へ直接置くと、この形になる）。
 * 補間は手前の `}` までしか取り除かれず、**残った `)}` が class トークンとして落ちる。**
 * 括弧を含むので任意値記法として落ちる。
 *
 * 入れ子を数える形にはしない。**`${` と `}` を数え始めると、文字列の中の括弧と
 * 区別するために式のパーサが要る**（冒頭の方針どおり、割に合わない）。
 * 書く側で**式の中で組み立てず、変数に受けてから補間する。**
 *
 * **クラスの中身は `check:component-classes` が出力の側から見ている**ので、
 * ここで見落としても原則3 は守られる（決定6-2）。
 *
 ...
 * （レジストリ配布では利用側リポジトリにコードがコピーされる。原則6）。
 *
