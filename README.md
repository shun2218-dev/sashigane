...
> ## ⚠️ 開発中：まだインストールできません
>
> 現在 **Phase 1（トークン層）**。トークンの生成・CSS / SCSS / JS の出力・色システムまでが
> 動いています。**未実装なのはコンポーネントとレジストリの配信**です。
> バージョンの規則は決めました（決定4-6）が、**タグはまだ1つも切っていません** —
> 配信が動いていないものをリリースしたことにしないためです。
...
（決定5-2 が端点をちょうど境界に解いているため、余裕がゼロです）。
