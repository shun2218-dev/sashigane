 ...
 * 選ばれているものだけを `tabIndex=0` にし、**中の移動は矢印キーで行う。**
 *
 * ## 矢印キーで移動すると、その場で中身が切り替わる
 *
 * WAI-ARIA はどちらも認めているが、**中身がその場にあるなら開く側**を勧めている。
 * このコンポーネントは中身を先に描いているので、開くのに待ちが無い。
 *
 * 押して初めて開く形（`activation="manual"`）も持つ。
 * **中身の読み込みに時間がかかるとき**は、矢印キーで移るだけで毎回読み込むことになる。
 *
 * ## 名前は利用側が渡す
 *
...
interface TabsState {
   ...
   * まだ何も選ばれていなければ、これを選ぶ。
   *
   * **並びの最初を選ぶために要る。** Tabs は子の顔ぶれを知らないので、
   * 最初のタブが自分で申告する。
   */
...
export function Tabs({
  ...
  /*
    **最初のタブが自分で申告する。** Tabs は子の顔ぶれを知らないので、
