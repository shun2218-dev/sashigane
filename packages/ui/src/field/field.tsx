 ...
 * 安定した名前を作るには `useId` が要り、**このコンポーネントがクライアント側になる。**
 * いまのコンポーネントはどれもサーバ側で描ける。
 *
 * 必須にしたことで、**書き忘れが型で止まる。** 忘れたまま描かれると、
 * 見た目は正常なのに読み上げがラベルと入力を結べない。
 *
 * **関連付けは Slot で子へ移す。** 子が1つだけという制約は `asChild` と同じ形で、
 * 仕組みも共有している——移し方が2つになると、壊れるまで誰も気づかない。
 *
 * **移し方は子を勝たせる。** つまり入力の側で `id` を書くと、
 * こちらが渡した `id` が消える。ラベルは元の `id` を指したままなので、
...
/**
 * 入力ひとつぶんのまとまり。**ラベル・説明・誤りを、入力に関連付ける。**
 *
 * ## 関連付けは Field が作る
 *
...
export interface FieldProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
   ...
   * ラベルと入力の並べ方。
   *
   * **`inline` はラベルが入力の右に来る。** チェックボックスとラジオがそれである——
   * 上に置くと、ラベルがどの選択肢のものか目で辿れない。
   *
   * 置き方をコンポーネントごとに持たせていない。持たせると**関連付けの経路が2つ**になり、
   * 片方だけ直したときにずれる。
   */
  layout?: 'stacked' | 'inline';
  /** 入力が要るかどうか。**ラベルにも入力にも届く** */
  required?: boolean;
  /**
   * 満たしていること。**印と境界で表す。**
   *
...
/** 関連付けに使う props。**入力の側で書かれると、こちらが渡したものが消える** */
...
export function Field({
    ...
    if (taken.length > 0) {
      throw new Error(
        `Field の中の入力に ${taken.join(' / ')} を書かないでください。` +
          'これらは Field がラベルと誤りに関連付けるために渡します——' +
  ...
  /*
    ...
    **中身の無い枠が増えるのを嫌って条件つきにしていた。** 代償が合っていない。
  */
  const controlNode = (
