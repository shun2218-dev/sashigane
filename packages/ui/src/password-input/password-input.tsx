 ...
 * ── 維持する側への覚書 ───────────────────────────────
 *
 * **`type` を入れ替えるだけである。** 器を差し替えない。
 *
 * 状態で器の有無を変えると React が中身を作り直し、**打っている最中に
 ...
 * サーバ側では描けない。
 *
 * 入れ替えるのは同じ要素の属性だけである。器ごと差し替えると、
 * **打っている最中にフォーカスが外れる。**
 *
 * ## 見せている状態を隠さない
 *
 * ボタンの札が「見せる」から「隠す」に変わる。
 * **読み上げは札の変化で状態を知る**ので、`aria-pressed` は重ねない。
 *
 * ## 中身は覚えない
 *
...
export function PasswordInput({
   ...
   * 欄の**外**（器の並びと重ね）にある。渡すと使われない余白が中に増える。
   */
