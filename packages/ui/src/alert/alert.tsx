 ...
 * トーンごとの既定のアイコンを持たないのはそのためで、
 * 持つと「色とアイコンがあるから文言は短くてよい」という方向へ倒れる。
 *
 * ## 読み上げの領域は既定で作らない
 *
 ...
 * **後から出したときだけ `live` を渡す。**
 *
 * ## 閉じるボタンに Button を使わない
 *
 * **色の保証が届かない。** 淡い塗りの上で読めることが保証されているのは
 * `on-{トーン}-subtle` だけである。実測（暗色）はこうなっている。
 *
 * |    | on-{トーン}-subtle | ghost の既定（accent） | トーンを揃えた場合 |
 * |---|---|---|---|
 * | 最小 | 5.78 | 4.32 | **3.95** |
 * | 最大 | 6.42 | 4.58 | 4.57 |
 *
 * Button は `ghost` でも `text-accent` を書く。**淡い塗りの上に置くと、
 * 本文より薄い字が並ぶ。** トーンを揃えても下がるだけである。
 *
 * だから**色を1つも書かないボタン**を自分で持ち、枠の前景を継ぐ。
 ...
 * 置かれた場所で決まる。**Alert はそれを知らない。**
 * レベルを固定すると、置いた先の階層が飛ぶ。
 *
...
const alert = cva('flex w-full items-start gap-3 rounded-xl border-1 border-border p-4', {
  variants: {
     ...
     * どのトーンで色を付けるか。**既定は中立。**
     *
     * **トーンは意味を運ばない。** 色の違いだけでは何が起きたか伝わらないので、
     * 文言だけで意味が通るようにする。
     */
...
export interface AlertProps
   ...
   * 見出し。**渡さなければ本文だけになる。**
   *
   * 結びつけ（`aria-labelledby`）は Alert が作る。
   */
  title?: ReactNode;
  /**
   * 頭に置くアイコン。**トーンごとの既定は持たない。**
   *
   * 色とアイコンで伝えようとすると文言が短くなる。**意味は文言が持つ。**
   */
  icon?: ReactNode;
  /**
   * 読み上げに知らせるか。**既定は知らせない。**
  ...
  live?: boolean;
  /**
   * 閉じたとき。**渡したときだけ閉じるボタンが出る。**
   *
   * 渡さなければ閉じられない。消してはいけない知らせがあるので、
   * 既定で消せる形にしていない。
   */
  onDismiss?: () => void;
  /** 閉じるボタンの名前。**アイコンだけでは何のボタンか読めない** */
  dismissLabel?: string;
  children?: ReactNode;
    ...
    >
      ...
      {onDismiss ? (
        <button
          ...
          onClick={onDismiss}
          /*
            **色を1つも書かない。** 枠の前景（`on-{トーン}-subtle` か面の段）を継ぐ。
