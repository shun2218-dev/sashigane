 ...
 * 実際に生成 CSS を grep して確かめてから、境界を升の側に置いた。
 *
 ...
 * 並べたものが文章とは限らないためである（札の一覧、設定の行）。
 *
 * ## 区切り線は升が持つ
 *
 * 器ではなく升が持つ。**最初の升だけ線を持たない**ので、上端に線が残らない。
 */
...
export interface ListProps
  ...
  /**
   * 升の間を線で区切るか。**設定の行や一覧で使う。**
   *
   * 線が間隔を担うので、選ぶと升の間隔は詰まる。
   */
  separated?: boolean;
  /**
   * 順序のある並びにするか。**`ol` になる。**
...
const item = cva('marker:text-muted', {
  variants: {
    /**
     * 上辺に線を持つか。**器が区切ると決めたときだけ。**
     *
     * 最初の升は持たない——**上端に線が残ると、器の枠に見える。**
     */
...
export interface ListItemProps extends LiHTMLAttributes<HTMLLIElement> {
  /**
   * 上辺に線を持つ。**器の `separated` に合わせる。**
   *
   * 器から自動では降りてこない——`li` は器の直下とは限らず、
