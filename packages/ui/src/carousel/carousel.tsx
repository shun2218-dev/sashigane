 ...
 *
 * **依存はこのコンポーネントを入れた利用者だけが持つ。** 配信 JSON は import から依存を導くので、
 * `@sashigane/carousel` にだけ載る。
 *
 * ## コンテキストで配っている
 *
 * この系のコンポーネントはほとんど状態を持たないが、**スクロールの状態は前後のボタンと位置表示が共有する。**
  ...
  */
  ...
  /*
    **止める手段が無ければ落とす**（WCAG 2.2.2）。
    ...
    説明に書くだけでは守られない。**黙って動き続けるほうが害が大きい**——
