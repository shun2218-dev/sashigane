 ...
 *
 * 線を描くのは入力を囲む枠で、枠はページの面の上にある。
 *
 * ## 1箇所に置いている理由
 *
 * 入力・複数行の入力・チェックボックス・ラジオが同じ線を描く。
...
/**
 * 属性と props から線の状態を決める。**コンポーネントごとに書かない。**
 *
 ...
 *
 * **輪郭で描く。** 境界の幅を変えると箱のサイズが変わり、
 * フォーカスのたびに下にあるものがずれる。輪郭は描画だけでサイズに関わらない。
 *
 * **通常の線も文字の役割である**（`outline-faint`）。境界の役割はどの段も
 * 自分が乗る面に対して 3:1 に届かない（`border-strong` でも 2.67。実測）。
 * 線が無いとコンポーネントだと分からないので、届かないと困る。
 *
