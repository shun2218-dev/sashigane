 ...
 * （押している最中の見た目は持たない／無効は面を暗くして表す、あたり）。
 *
 * **パスでは書かない。** このファイルはレジストリ配信で利用側リポジトリへコピーされるので、
 * リポジトリ相対のパスはコピー先で必ず壊れる。書くなら絶対 URL にする。
 *
 * ## 踏みやすい罠が3つある
 *
 ...
 *
 * **ローディング中は暗くしない。** 無効は「できない」で、ローディング中は「いま起きている」である。
 * 面を暗くすると2つが同じ見た目になり、区別が付かなくなる。
 ...
 * 面の仕組みに乗るので、**塗り・淡い塗り・枠・文字だけのどれでも同じ形で暗くなる**——
 ...
 * ## ローディング中は暗くしない
 *
 * 無効は「できない」で、ローディング中は「いま起きている」である。
 * **同じ見た目にすると区別が付かない**ので、ローディング中は塗りをそのまま残す。
 *
 * 押せなくはする。スピナーが回っているだけで押せてしまうと、二重に送れてしまう。
 *
 * **両方渡したときはローディング中が勝つ。** 暗くなった上でスピナーが回ると
 * 「できないのに待っている」になり、意味が通らない。
 *
 * ## 枠はスピナーのぶんだけ横に伸びる
 *
 * 文字は残すので、スピナーが増えたぶん幅が変わる。**押した瞬間に隣が動く。**
 * ずれてほしくない場所では、枠の幅を先に決めておく。
 *
 * アイコンだけのボタンではスピナーが置き換えるので伸びない。
 *
...
const button = cva(
    ...
    FOCUS_RING,
  {
    variants: {
       ...
       */
      /*
       * `outline` の境界は**中立色である。** トーンの色にしない——
       * **色はトーンに、境界は面の骨格に属する。** 揃え忘れではない。
       *
      ...
      /** どのトーンで塗るか。**塗りを持つトーンすべて** */
      tone: { accent: '', danger: '', warning: '', success: '', info: '' },
      /**
       * 無効。**不透明度は使わない**。
       * `inset` の面を宣言して暗くし、文字を `text-faint` にする。
       */
       ...
       *
       * 正方形に固定していないのは、**固定できるサイズがスケールに無い**ためである。
...
export type ButtonProps = IconOnly &
  (
    | (ButtonBase & {
      ...
      /**
       * 押せない状態。**不透明度では表さない**。
       * 面を `inset` に宣言して暗くし、文字を `text-faint` にする。
       */
      disabled?: boolean;
      /**
       * ローディング中。**スピナーを出し、押せなくする。**
       *
       * **暗くしない。** 無効は「できない」で、ローディング中は「いま起きている」である。
       * 同じ見た目にすると区別が付かない。
       *
       * 文字はそのまま残す。スピナーだけにすると、何を待っているのか分からなくなる——
       * ただし**アイコンだけのボタンではスピナーが置き換える。**
       */
      loading?: boolean;
    })
  | (ButtonBase & {
...
export function Button({
   ...
   * 塞ぐと**その書き方ができなくなる。**
   *
   * 暗くなった上でスピナーが回ると「できないのに待っている」になり、意味が通らない。
   * 暗くするだけにすると、**何かが起きていることが消える。**
   * ローディング中の方が短く、いま起きていることなので、そちらを見せる。
   */
  ...
  const shared = {
    ...
    // **無効のときだけ面を宣言する。** 面の仕掛けが背景と前景を同時に暗くする。
    // **ローディング中は暗くしない**——「できない」と「いま起きている」は別である
  ...
  /*
   * ローディング中の中身。**スピナーは読み上げから隠す**——
   * ボタンは文字か `aria-label` で既に名前を持っており、
   * 隠さないと「ローディング中 ローディング中」と二重に読まれる。
   *
   * **アイコンだけのボタンではスピナーが置き換える。** 並べると枠が横に伸びて、
   * 正方形でなくなる。文字のボタンでは残す——
   * スピナーだけにすると、何を待っているのか分からなくなる。
   */
  ...
  return (
    <button
      type="button"
      // **押せなくする。** スピナーが回っているだけで押せると、二重に送れてしまう
