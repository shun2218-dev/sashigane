 ...
 * ── 維持する側への覚書 ───────────────────────────────
 *
 * **素の `details` / `summary` に乗っている。** 開閉もキーボードも読み上げも、
 * ブラウザが既に持っている。`aria-expanded` を自分で書かない——
 * 書くと、ブラウザの申告と二重になる。
 *
 * **一度に1つだけ開く形も、素の仕組みで解ける**（`details` の `name`）。
 * 状態を持たないので、hooks もコンテキストも要らない。
 *
 * **名前は利用側が渡す。** 枠が作って配る形も試したが、
 * 安定した名前を作るには `useId` が要り、**枠がクライアント側のコンポーネントになる。**
 * いまのコンポーネントはどれもサーバ側で描けるので、そこを崩さない。
 *
 ...
 * 面・塗り・オーバーレイ・スケルトンと同じ形で、規則は `tokens.css` が持つ。
 *
