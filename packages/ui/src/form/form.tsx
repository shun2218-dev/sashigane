 ...
 *
 * ## フォーカスを移すために、クライアント側のコンポーネントになっている
 *
 * 他のコンポーネントと違ってサーバ側で描けない。**送信の後に DOM を見る**ためである。
 ...
 * 待たずに探すと、まだどの欄にも誤りが付いていない。
 *
 * **Promise を返さない非同期の検証には追随しない。** 待ち続ける形にはしていない——
 * いつやめるかを決められず、**時間が経ってからフォーカスを奪うのは、移さないより悪い。**
 * そのころ利用者は別の場所を触っている。
 *
 * 待ち方は2段ある——**利用側の処理（Promise を返すならその解決）**と、
...
/** 誤りが付いた欄。**利用側が何で検証したかに依らない** */
const INVALID = '[aria-invalid="true"]';

/**
 * フォーカスを持てるもの。**誤りが付くものがフォーカスを持てるとは限らない**——
 * ラジオのグループは `fieldset` に付くが、`fieldset` にフォーカスは乗らない。
 */
...
/** フォーカスを移す先。**誤りが付いたものか、その中の最初のフォーカスを持てるもの** */
...
export function Form({
  ...
  const handleSubmit: NonNullable<FormHTMLAttributes<HTMLFormElement>['onSubmit']> = async (
     ...
     * まだどの欄にも誤りが付いていない。
     *
     * Promise を返す書き方（`handleSubmit()` など）はその解決を待つ。
