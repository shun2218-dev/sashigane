...
export const toTokensCss = (palette: Palette): string =>
  [
    ...
    '}',
    '',
    '/* 折りたたみの開閉。**素の details に乗せたまま、高さを動かす。**',
    '',
    '   閉じているあいだ、ブラウザは中身を描画しない。0 から実際の高さへ動かすには',
    '   `::details-content` に触り、`interpolate-size` で auto を補間できるようにする。',
    '   **`interpolate-size` は継承する**ので :root に一度書けば届く。',
    '',
    '   **宣言した details だけが動く。** 面・塗り・覆い・骨組みと同じ形である。',
    '     <details data-sg-collapse>...</details>',
    '',
    '   `content-visibility` を一緒に遷移させるのは、**閉じるときにも動かすため**である。',
    '   これが無いと、閉じた瞬間に中身が消えて高さだけが縮む。',
    '',
    ...
    '}',
    '',
    '[data-sg-collapse]::details-content {',
    '  block-size: 0;',
    '  overflow: hidden;',
    '  transition:',
    '    block-size var(--sg-duration-2) ease-in-out,',
    '    content-visibility var(--sg-duration-2) ease-in-out allow-discrete;',
    '}',
    '',
    '[data-sg-collapse][open]::details-content {',
    '  block-size: auto;',
    '}',
    '',
    ...
    '}',
    '',
    '/* 出入りの動き。**覆いを持つものは、覆いと一緒に出入りする。**',
    '',
    '   閉じるときが難しい。`close()` は表示を即座に消すので、',
    '   何もしないと**消えてから動く**ことになり、動きが見えない。',
    '   `display` と `overlay` を allow-discrete で遷移させると、',
    '   **動き終わるまで最前面の層に留まる。**',
    '',
    '   Tailwind 側には出口を作っていない。`transition-property` に',
    '   `display` と `overlay` を並べる書き方が、クラス名の規則で書けないためである。',
    '',
    '   **動きを減らす設定を尊重するのは利用側の責務ではない。** ここで止める。 */',
    '[data-sg-scrim],',
    '[data-sg-scrim]::backdrop {',
    '  opacity: 0;',
    '  transition:',
    '    opacity var(--sg-duration-2) ease-in-out,',
    '    display var(--sg-duration-2) ease-in-out allow-discrete,',
    '    overlay var(--sg-duration-2) ease-in-out allow-discrete;',
    '}',
    '',
    '[data-sg-scrim][open],',
    '[data-sg-scrim][open]::backdrop {',
    '  opacity: 1;',
    '}',
    '',
    '/* 開いた瞬間の始点。**これが無いと、開くときだけ動かない**——',
    '   表示され始めた要素には遷移の始点が無いためである */',
    '@starting-style {',
    '  [data-sg-scrim][open],',
    '  [data-sg-scrim][open]::backdrop {',
    '    opacity: 0;',
    ...
    '}',
    '',
    '/* 出入りの動き。**宣言したものが、現れるときと消えるときに動く。**',
    '',
    '     <li data-sg-appear>…</li>',
    '     <li data-sg-appear data-sg-leaving>…</li>   ← 消えかけ',
    '',
    '   **消えかけは、外す前の状態である。** 動きが終わってから外す。',
    '   長さは CSS にしかない——待つ側は `Animation` の終わりを見るので、',
    '   **同じ長さを JS 側に写さなくてよい。**',
    '',
    '   消えるときに高さを 0 にするのは、**後ろに並んでいるものが跳ねないため**である。',
    '',
    '   **動きを減らす設定では止める。** 出入りは飾りであって、情報ではない。',
    '   ただし**消えかけは残す**——止めた結果いつまでも消えないのでは困るので、',
    '   長さを詰めて即座に終わらせる。 */',
    ...APPEAR_KEYFRAMES,
    '',
    ...DISAPPEAR_KEYFRAMES,
    '',
    '[data-sg-appear] {',
    '  animation: sg-appear var(--sg-duration-2) ease-out;',
    '}',
    '',
    '[data-sg-appear][data-sg-leaving] {',
    '  animation: sg-disappear var(--sg-duration-2) ease-in forwards;',
    '}',
    '',
    '@media (prefers-reduced-motion: reduce) {',
    '  [data-sg-appear] {',
    ...
    '',
    '  [data-sg-appear][data-sg-leaving] {',
    '    animation-name: sg-disappear;',
    '    animation-duration: 1ms;',
    '  }',
    '}',
    '',
    '/* 残り時間のゲージ。**宣言した要素が、満ちた状態から空へまっすぐ減る。**',
    '',
    '   長さは差し込み口から取る。**書かなければ通知の滞在時間**になる:',
    '     <span data-sg-gauge></span>',
    '     <span data-sg-gauge style="--sg-gauge-duration: 8000ms"></span>',
    '',
    '   **止めるのは属性である。** ポインタが乗っているあいだ数えない、を表す:',
    '     <span data-sg-gauge data-sg-gauge-paused></span>',
    '',
    '   加減速は付けない。**時間そのものを表しているので、速さが変わると嘘になる。**',
    '',
    '   **動きを減らす設定でも止めない。** これは飾りではなく、',
    '   あと何秒で消えるかという情報である。止めると読み取れなくなる。 */',
    ...GAUGE_KEYFRAMES,
    '',
    '[data-sg-gauge] {',
    '  transform-origin: left center;',
    '  animation: sg-gauge var(--sg-gauge-duration, var(--sg-duration-notice)) linear forwards;',
    '}',
    '',
    '[data-sg-gauge-paused] {',
