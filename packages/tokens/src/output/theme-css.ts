...
export const toThemeCss = (palette: Palette): string =>
  [
    ...outputHeader('block', 'Tailwind v4 用アダプタ。', palette, [
      ...
      'テーマ由来の値は1つも残らない。',
      '写像していない名前空間（--container-* など）は利用側の責務である。',
      '',
    ...
    '',
    '@theme inline {',
    '  /* 名前空間を全部落とす。列挙すると書き漏らしたものが黙って通る。',
    ...
    '     使われている場所が無く、写像先の需要が無い */',
    ...elevationRoles.map((r) => `  --shadow-${r}: var(--sg-elevation-${r});`),
    '',
    '  /* duration — 名前は ms の値に合わせる。索引ではなく実際の値を表す規約である。',
    '     索引で写像すると duration-2 が 200ms になり、素の Tailwind（2ms）と食い違う。',
    '     **小数の鍵は使える**（実測。--transition-duration-141\\.4 が duration-141.4 を作る）',
    '     ので、√2 刻みのスケールが全段そのまま届く。',
    '',
    '     **写像するのは遷移の段だけ。** 遷移とループは知覚上の制約が違う',
    '     別のスケールである。同じ名前空間に混ぜると、ループの値を hover の遷移に',
    '     当てられてしまい、**別スケールにした理由が Tailwind の経路で消える。**',
    '     ループ周期に出口は要らない——アニメーションは data-sg-skeleton の1本である',
    '     素の CSS からは var(--sg-duration-loop-*) で読める。',
    '',
    '     delay も写像しない。使われている場所が1件も無い */',
    ...durationTransition.map(
      (ms, i) => `  --transition-duration-${msKey(ms)}: var(--sg-duration-${i});`,
    ),
    '',
    '  /* container — 器の幅（max-w-* / w-* / min-w-*）。',
    '',
    '     **滞在時間は写像しない。** 知らせが留まる長さは CSS が使う値ではなく、',
    '     JS が数える値である。`duration-*` へ出すと遷移として当てられ、',
    '     **別スケールにした理由が Tailwind の経路で消える。**',
    '     素の CSS と JS からは var(--sg-duration-dwell-*) で読める。',
    '',
    '     **名前は rem の値に合わせる。** 素の t シャツ語彙（max-w-lg など）は',
    '     値が一致しないので使わない。索引にすると max-w-2 が素の Tailwind の',
    '     spacing 由来の名前と衝突する */',
    ...width.map((v, i) => `  --container-${Math.round(v)}rem: var(--sg-width-${i});`),
    '',
    '  /* text — 素の t シャツ語彙は値が一致しないので使わない。',
    '     セマンティック役割名を使い、行高を対で束ねる */',
    ...
    '     **同じものに2つの道があって片方だけが安全**という形は、面でも',
    '     hover でも退けてきた。忘れても何も言われないので危ない。',
    '',
