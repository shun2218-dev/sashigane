 ...
 * ## なぜ共通化するか
 *
 * レジストリ方式（原則6）では、生成物は**利用側リポジトリへコピーされる。**
 * コピー先に `docs/decisions.md` は無く、「原則3」という参照も解決できない。
 ...
 * 利用側が「更新された」と誤認する。
 *
 * **バージョンは入れる**（決定4-6）。レジストリ方式では生成物が利用側リポジトリへコピーされるので、
 * **コピー先で「どのスナップショットを持っているか」を知る手がかりがこれしかない。**
